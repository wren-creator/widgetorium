# Training scenarios: Widgetorium (instructor answer key)

Each entry follows the same shape: what is broken, how ZAP finds it, which tool
confirms or exploits it, and the fix. Work through them in order or jump to
...
**Fix:** parameterised query with a bound `LIKE` term (`WHERE name LIKE
CONCAT('%', :q, '%')` with `:q` bound).
...
**Fix:** parameterised query; look the user up by username only, then
`password_verify()` against a real password hash (bcrypt/argon2).
...
**Fix:** parameterise every query, including ones built from data already in the
database. Stored data is still untrusted input. (`SECOND_ORDER_SINK=0` swaps in
the safe `PDO::quote` variant for comparison.)
...
`--data '{"id":"7*","price":"1"}'`.
**Fix:** parameterised `UPDATE`; authorise against the session (owner or admin,
not "is logged in"); revoke `FILE` from the app user and set `secure_file_priv`
to a locked-down directory.
...
**Fix:** certificate from a trusted CA (or a managed internal CA for an
internal-only app) with a SAN that matches the hostname.
...
-noout -dates`.
**Fix:** renewal automation and expiry monitoring. This is the "how did nobody
notice" lesson.
...
**Fix:** disable everything below TLS 1.2, cut the cipher list to modern AEAD
suites, 2048-bit or better DH / ECDHE. (`WEAK_TLS=0` restores sane defaults.)
...
over HTTPS. (`SEND_HSTS=1` adds it.)
...
grep -i api_key`.
**Fix:** never deploy `.git` into a web root (build artefacts only, or deny the
path at the server). Rotate any secret that has ever been committed.
...
**Fix:** `WHERE id = :id AND customer_id = <session user>`, or an explicit
ownership check before rendering. Non-enumerable ids help but are not the fix.
...
**Fix:** encode on output at render time (`htmlspecialchars` / a templating
engine that auto-escapes). Add a Content-Security-Policy. Set `HttpOnly` on the
session cookie.
...
**Fix:** whitelist extensions, validate content server-side, `basename()` the
filename, store uploads outside the web root and serve them through a handler,
...
**Fix:** cryptographically random tokens (`random_bytes`), or just use the
platform session manager. Bind the session to more than a guessable string.
...
bugs 1 and 3.
**Fix:** generic error pages in production, `display_errors = Off`, log the
detail server-side.
...
**Fix:** disable anonymous login. If it is genuinely required, read-only, and
never inside a directory that anything else serves.
...
**Fix:** no default credentials ship. Enforce a real password policy on service
accounts, or use keys / SFTP.
...
**Fix:** `chroot_local_user=YES` with a correctly owned, non-writable chroot
target; verify against a fresh config rather than patching an old one.
...
**Fix:** FTPS (`ssl_enable=YES`, force it for logins and data) or move to SFTP.
...
**Fix:** never let an FTP-writable directory sit inside a web root; enforce
file-type restrictions regardless of upload path; disable script execution in
upload directories.
